...
export class RosHelper {
    ...
    ): Promise<Observable<{ pin: number; domain: number; value: number }>> {
    
    const topicPath = await ros2Client.getTopicPath(
        true,
        "analog_status",
        URCAP_ROS_NAMESPACE
        );
    ...
    return ros2Client.createSubscription(
        "urinterfaces/msg/Analog",
        topicPath,
        qosProfile
        );
    }
    ...
    //         {seconds: 0, nanoseconds: 0},
    //         false);
    //     return ros2Client.createSubscription('urinterfaces/msg/DigitalIOStateStamped.msg', topicPath, qosProfile);
    // }
